local M = {}

local palette = require("eyes.palette")

M.setup = function()
	vim.g.terminal_color_0 = palette.hex04
	vim.g.terminal_color_1 = palette.hex06
	vim.g.terminal_color_2 = palette.hex08
	vim.g.terminal_color_3 = palette.hex10
	vim.g.terminal_color_4 = palette.hex04
	vim.g.terminal_color_5 = palette.hex06
	vim.g.terminal_color_6 = palette.hex08
	vim.g.terminal_color_7 = palette.hex10
	vim.g.terminal_color_8 = palette.hex04
	vim.g.terminal_color_9 = palette.hex06
	vim.g.terminal_color_10 = palette.hex08
	vim.g.terminal_color_11 = palette.hex10
	vim.g.terminal_color_12 = palette.hex04
	vim.g.terminal_color_13 = palette.hex06
	vim.g.terminal_color_14 = palette.hex08
	vim.g.terminal_color_15 = palette.hex10
end

return M
